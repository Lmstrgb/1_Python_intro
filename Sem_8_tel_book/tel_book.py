...
def input_data()->list:
    ...
    user.append(input("Введите телефон "))
...
def export_phone_dir(phone_dir: dict, file_name: str):
    MAIN_DIR = join(abspath(dirname(__file__)), 'Data') # Изменен путь к папке для экспорта, добавлено 'Data'
    full_name = join(MAIN_DIR, file_name+'.txt')
    with open(full_name, mode='w', encoding='utf-8') as file:
            for idc in phone_dir:   # Внесено изменение
              file.write(f"{idc}#{'#'.join(phone_dir[idc])} \n") # Внесено изменение
...
def print_dict(phone_dir: dict):
    for idc in phone_dir:
        print(f"{idc}: {' '.join(phone_dir[idc])}") # Внесено изменение 
    # for idc, user in phone_dir.items():
...
def print_once(phone_dir: dict, id: int):
    #user = phone_dir[id]
    #for idc in phone_dir:
    print(f"{' '.join(phone_dir[id])}") # внесено изменение
    # print(f"{user[0]} {user[1]} {user[2]} {user[3]}")

def import_phone_dir(phone_dir, file_name):
    MAIN_DIR = join(abspath(dirname(__file__)), 'Data') # добавлено Data
